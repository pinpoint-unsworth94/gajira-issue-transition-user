...
class Jira {
  ...
  async getIssueChangeLog (issueId) {
    try {
      const res = await this.fetch('getIssueChangeLog', {
        pathname: `/rest/api/3/issue/${issueId}/changelog`
      })
  ...
  async getUserGroups (accountId) {
    try {
      const res = await this.fetch('getUserGroups', {
        pathname: `/rest/api/3/user/groups`,
        query: {
          accountId: accountId
        }
      })
